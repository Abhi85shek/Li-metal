...
router.post("/storelocal", async (req,res)=>{
    ...
    db.query("INSERT INTO limetalorders (supplierName,customerName,totalAmount,docNumber,detailType,line,apAccountRefname,apAccountRefvalue,vendorRefname,vendorRefValue,shipToName,shipToValue,primaryApprover,secondaryApprover,primaryApproved,secondaryApproved,overallStatus,createdBy,vendorId) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    [supplierName,customerName,TotalAmt,DocNumber,"ItemBasedExpenseLineDetail",JSON.stringify(Line),apName,apValue,VendorRef.name,VendorRef.value,shipName,shipValue,primaryApprover,secondaryApprover,0,0,0,userId,vendorId],(err,result)=>{
        if(err)
            {
